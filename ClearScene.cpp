...
void ClearScene::Update() {
	...
	if (transition_->GetIsChangeScene()) {
		...
		if (
			(transition_->GetFadeIn() && transition_->GetNextScene() == TITLE) ||
			(transition_->GetFadeOut() && transition_->GetNextScene() == CLEAR)
			) {
			transition_->Update();
		}
		// ゲームシーンからのフェードアウト終了でシーン遷移を止める
		else if (transition_->GetFadeIn() && transition_->GetNextScene() == CLEAR) {
